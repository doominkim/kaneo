...
## 현재 상태 — 원장 소프트 삭제·복구와 용어 삭제(`agent.13`)까지 운영 반영 완료
...
| 대상 | 확정 상태 | 근거 |
|---|---|---|
| Kaneo 코드 | `agent-layer`의 `831f502d` push 완료 | `git` 원격 브랜치 확인 |
| 이미지 | `ghcr.io/doominkim/kaneo:2.22.0-agent.13` 빌드 성공 | [GitHub Actions run 33723736611](https://github.com/doominkim/kaneo/actions/runs/33723736611) |
| GitOps manifest | platform `main`의 `224cc5a` | 이미지 태그 `agent.13`, SealedSecret과 공개 S3 환경값 포함 |
| TLS vhost | sandbox `main`의 `5dc74e2` | `files.kit.io.kr` 전용 nginx vhost |
| Argo / Pod | `kaneo-prod` Synced, Healthy, image `agent.13`, 1/1 Ready, restart 0, agent-layer 마이그레이션 0000~0006 적용 | 운영 클러스터 실측 (2026-09-03 15:45 KST) |
...
- `2.22.0-agent.2`~`agent.12`는 더 이상 배포 대상이 아니다. 현재는 `2.22.0-agent.13`이다.
- `apps/kaneo/prod.yaml`은 미커밋/빈 `sealedEnv` 상태가 아니다. `224cc5a`가 운영에 반영되었다.
