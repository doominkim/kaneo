...
- **knowledge item**: one named fact in the Agent Layer — a canonical name plus definition, aliases, do-not-confuse list, and DB or code anchors. A write applies immediately, an agent's included; agent and API-key writes stay unreviewed until a person with read access opens the item, and people dispute or soft-delete wrong ones. Agents read live confirmed items together with a `reviewed` flag. Named rules and mappings qualify, not only single words.
- **domain page**: workspace-scoped prose about a part of the business. Unnamed and without a review flag, so it always carries its last author and update time.
